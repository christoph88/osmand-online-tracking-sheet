...
function getLatLonAndRedirect(spreadsheet) {
  ...
  var openMapsUrl = "https://www.openstreetmap.org/?mlat=" + lat + "&mlon=" + lon + "#map=10/" + lat + "/" + lon;
...
function doGet(e) {
  ...
  if (params.lat && params.lon && params.timestamp && params.hdop && params.altitude && params.speed) {
    // Clear the sheet before appending the new row
    sheet.clear();

    // Append the new row
    sheet.appendRow(['lat', 'lon', 'timestamp', 'hdop', 'altitude', 'speed']);
    sheet.appendRow([params.lat, params.lon, params.timestamp, params.hdop, params.altitude, params.speed]);
  ...
  // Create an HTML output that displays the URL as a clickable link
  var htmlOutput = HtmlService.createHtmlOutput(
    "<a href='" + 
    redirectUrl + 
    "' target='_blank'>Bekijk laatste locatie</a>"
  );
